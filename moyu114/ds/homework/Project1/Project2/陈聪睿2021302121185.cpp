...
int loopprocess(int a[])
{
	...
	for (i = 0;i < N;i++) {
		for (j = i;j < N;j++) {
			...
			if (ThisSum > result) {    //判断现在的结果和现在的和的大小，若现在的和大则更新
				result = ThisSum;
			}
		}
	}
...
int main()
{
	...
	cout << resultloop <<" ";  //处理1000个数据用了0.4233s 
	resultdev = devideprocess(a,0,N-1);
	cout << resultdev << " ";    //处理1000个数据用了0.05064s 
	resultline = lineprocess(a);
	cout << resultline << " ";   //处理1000个数据用了0.0481s
